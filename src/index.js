...
import ApiServes from './api-serves';
import { onHitsMarkup } from './onHitsMarkup';

const apiServes = new ApiServes();
...
formEl.addEventListener('submit', onSerch);
...
function onSerch(e) {
  ...
  apiServes.qury = e.currentTarget.elements.searchQuery.value;
  if(apiServes.qury === ''){
    return Notiflix.Notify.failure('введи корректное имя в поиске');
  }
  apiServes.resetPage();
  apiServes.fetchArticles().then(articles => {
    clearArticlesDiv()
    appendArticelsMarcup(articles);
    lightbox.refresh();
    onhiddenBtn()
...
function appendArticelsMarcup(articles) {
  divEl.insertAdjacentHTML('beforeend', onHitsMarkup(articles));
}

function clearArticlesDiv() {
  divEl.innerHTML = ''
}
